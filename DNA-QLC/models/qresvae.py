    ...
    def compress(self, enc_features):
        ...
        for i, block in enumerate(self.dec_blocks):
            if hasattr(block, 'compress'):
                ...
                strs_16 = binascii.b2a_hex(strs_batch[0])
                strs_16 = strs_16.decode('utf-8')
                str_b = []
                for i in range(len(strs_16)):
                    strs = data_handle.mybin(strs_16[i])
                    str_b.append(strs)
                str_b = ''.join(str_b)
                strings_all.append(str_b)
            else:
                feature = block(feature)
...
class HierarchicalVAE(nn.Module):
    ...
    def compress(self, im):
        ...
        min_res = min(enc_features.keys())
        tup = tuple(enc_features[min_res].shape)
        end=[]
